...
def test_has_valid_extension():
    ...
    valid_fnames = ["some.figure/fig.png", "fig.png", "fig.pdf", "fig.tiff", "fig.eps", "fig.has.other.dots.png"]

    invalid_fnames = ["some.figure/fig.aklwdmawk", "fig.has.other.dots.jpgpng", "test.test"]

    assert all(src.utils.plotting.has_valid_extension(fig, valid_fname) for valid_fname in valid_fnames)

    assert not any(src.utils.plotting.has_valid_extension(fig, invalid_fname) for invalid_fname in invalid_fnames)
    ...
    if presenter == "invalid":
        ...
        if presenter == "show":
            ...
            if presenter is pytorch_lightning.loggers.logger.DummyLogger:
                ...
                lg_conf = OmegaConf.load(os.path.join(pyrootutils.find_root(), "configs", "logger", presenter_name + ".yaml"))
            ...
            if presenter is pytorch_lightning.loggers.MLFlowLogger:
                ...
                assert logger.experiment.list_artifacts(logger.run_id)[-1].path == f"{test_title}.png"
                ...
                if platform.system() == "Windows":
                    assert logger.experiment.list_artifacts(logger.run_id)[-1].path == os.path.basename(new_fname)
                else:
                    assert logger.experiment.list_artifacts(logger.run_id)[-1].is_dir
                    assert logger.experiment.list_artifacts(logger.run_id)[-1].path == os.path.dirname(new_fname)
