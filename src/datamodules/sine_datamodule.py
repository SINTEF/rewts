        ...
        if not self.data_train and not self.data_val and not self.data_test:
            ...
            if not isinstance(self.hparams.data_args, list):
                ...
                if isinstance(self.hparams.chunk_idx, int):
                    ...
                    self.hparams.data_args = [
                        self.hparams.data_args[idx] for idx in self.hparams.chunk_idx
                    ]
                else:
                    raise ValueError
            ...
            for data_args in self.hparams.data_args:
                ...
                segment = data_args["amplitude"] * np.sin(
                    data_args["frequency"] * segment_t + data_args["phase"]
                )
            ...
            if len(data_segments) > 1 and self.hparams.split_per_segment:
                assert all(
                    (isinstance(v, float) or v is None)
                    or isinstance(v, (list, {}.values().__class__))
                    and all((isinstance(vs, float) or vs is None) for vs in v)
                    for v in self.hparams.train_val_test_split.values()
                ), "Only float splits are supported with split_per_segment"
                splits = self.process_train_val_test_split(
                    darts.timeseries.TimeSeries.from_dataframe(self.data),
                ...
                for segment in data_segments:
                    for split_name, split_values in splits.items():
                        new_splits[split_name].append(
                            [
                                round(segment.shape[0] * split_values[0]) + base_index,
...
if __name__ == "__main__":
    ...
    cfg = src.utils.initialize_hydra(
        os.path.join(os.pardir, os.pardir, "configs", "datamodule", "sine.yaml"),
