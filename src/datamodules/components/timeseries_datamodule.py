...
class TimeSeriesDataModule(LightningDataModule):
    ...
    def _finalize_setup(self, load_dir: Optional[str] = None):
        ...
        for dataset_name in self.data:
            ...
            # ensure compatibility with timeseries generated by darts which will use the name "time"
            if self.data[dataset_name].time_dim != "time":
                self.data[dataset_name]._xa = self.data[dataset_name]._xa.rename(
                    {self.data[dataset_name].time_dim: "time"}
                )
                self.data[dataset_name]._time_dim = "time"
