...
def _assert_compatible_with_index(value: Union[int, pd.Timestamp, float], index: Union[pd.RangeIndex, pd.DatetimeIndex]):
    if isinstance(value, float):
        return
    elif isinstance(value, int):
        assert isinstance(index, (pd.RangeIndex, pd.DatetimeIndex)), "An int-object is only compatible with pd.RangeIndex. You can use the timeseries.get_index_at_point function to convert."
    elif isinstance(value, (str, pd.Timestamp)):
        assert isinstance(index, pd.DatetimeIndex), "A Timestamp-object is only compatible with pd.DatetimeIndex. You can use the timeseries.get_timestamp_at_point to convert."
    else:
        raise ValueError


def _assert_valid_type_and_index(data):
    assert isinstance(data, _SUPPORTED_DATA_TYPES), f"The supported types for self.data are {_SUPPORTED_DATA_TYPES}, you have {type(data)}"
    ...
    assert isinstance(index, _VALID_INDEX_TYPES), f"The supported types of data indexes are {_VALID_INDEX_TYPES}, you have {type(index)}"
...
class TimeSeriesDataModule(LightningDataModule):
    ...
    def __init__(  # TODO: add support for choosing which factory for timeseries?
        ...
        num_loader_workers: int = 0,  #TODO: decide if best to set these here or in a fit config
    ...
    def setup(self, stage: Optional[str] = None, load_dir: Optional[str] = None) -> None:
        ...
        The setup function processes the data and splits it into train, val and test sets as configured by the
        arguments to the __init__ method. It also applies the processing pipeline to the data.

        This function can be called directly if a data source has been configured. If not, you need to make a subclass
        in which you override the setup function.
        ...
        :param load_dir: The folder to which the state of the datamodule is saved for later reproduction
            (e.g. fitted scalers etc.).

        :return: None
        """
        assert self.hparams.data_source is not None, "This function can only be called directly if a data source has been configured. Either configure a data source to be loaded, or make a subclass in which you override the setup function."
    ...
    def _finalize_setup(self, load_dir: Optional[str] = None):
        ...
        assert isinstance(self.hparams.freq, _SUPPORTED_FREQ_TYPES), f"The supported types for freq are {_SUPPORTED_FREQ_TYPES}, you have {type(self.hparams.freq)}"

        if load_dir is not None:
            self.load_state(load_dir)
        else:
            if self.hparams.processing_pipeline is not None:
                assert self.hparams.train_val_test_split is None or "train" in self.hparams.train_val_test_split, \
        ...
        if not len(all_data_variables) == len(set(all_data_variables)):
            ...
            raise ValueError(f"data_variables must have unique entries. The variable(s) {dupes} appears in multiple entries.")
        ...
        if self.hparams.data_variables.get("actual_anomalies", None) is not None:
            if isinstance(self.data, pd.DataFrame):
                ...
                self.labels = darts.timeseries.TimeSeries.from_dataframe(self.data, value_cols=self.hparams.data_variables["actual_anomalies"], freq=self.hparams.freq)
            elif isinstance(self.data, darts.timeseries.TimeSeries):
                raise NotImplementedError
                #self.labels = self.data["actual_anomalies"]
            else:
                raise ValueError
            self.labels = self.labels.astype(np.dtype(getattr(np, f"float{self.hparams.precision}")))
            self.labels = darts.utils.missing_values.fill_missing_values(self.labels, fill=0.0)  # make configurable fill_value?
        # TODO: component_wise
        else:
        ...
        if self.hparams.resample is not None:
            ...
            for key, value in self.hparams.resample.items():
                if key not in _VALID_RESAMPLE_KEYS:
                    log.info(f"Unrecognized argument {key} given for resample. The argument is ignored.")
                elif key in required_keys:
                    required_keys.remove(key)
            assert len(required_keys) == 0, f"The following required keys are missing from resample {required_keys}"
            
            self.data = self.resample_dataset(self.data, **self.hparams.resample)
        
        if isinstance(self.data, pd.DataFrame):
            self.data.columns.name = None  # fix potential bug with Timeseries.from_dataset
            self.data = darts.timeseries.TimeSeries.from_dataframe(self.data,
                                                                   value_cols=all_data_variables,
                                                                   fill_missing_dates=True,
                                                                   freq=self.hparams.freq)  # TODO: add support for other arguments

        self.data = self.set_dataset_precision(self.data, precision=self.hparams.precision)

        self.hparams.train_val_test_split = self.process_train_val_test_split(self.data,
                                                                              self.hparams.train_val_test_split)

        for split_name, split_data in self.split_dataset(self.data, self.hparams.train_val_test_split).items():
        ...
        for split_name in ["train", "val", "test"]:
            ...
            if self.hparams.processing_pipeline is not None:
                if split_name == "train" and not getattr(self.hparams.processing_pipeline, "_fit_called", False):
                    ...
                    if not getattr(self.hparams.processing_pipeline, "_fit_called", False):
                        if self.data_train is None:
                            raise RuntimeError("A pipeline has been configured, but no training set has been provided on which it can be fitted. Either pass load_dir containing the state of a pipeline or configure a training set.")
                        else:
                            raise RuntimeError("processing_pipeline.transform was called before it was fitted. Ensure datamodule configuration is correct. Contact developer as this is an unexpected error.")
                    split_data = self.transform_data(split_data)

            split_data_seq = darts.utils.utils.series2seq(split_data)
            split_series = {k: [] if self.hparams.data_variables.get(k, None) else None for k in ["target", "past_covariates", "future_covariates", "actual_anomalies"]}
            ...
            for split_data in split_data_seq:
                ...
                if self.hparams.check_for_nan:
                    if np.any(np.isnan(split_data.all_values())):
                        raise ValueError(f"The {split_name} dataset contains nan-values and the check_for_nan attribute is set to True. Please check if data-processing pipeline is configured correctly.")
                    if split_labels is not None and np.any(np.isnan(split_labels.all_values())):
                        raise ValueError(f"The labels for {split_name} dataset contains nan-values and the check_for_nan attribute is set to True. Please check if data-processing pipeline is configured correctly.")
                ...
                for series_name in ["target", "past_covariates", "future_covariates"]:
                    if self.hparams.data_variables.get(series_name, None) is not None:
                        split_series[series_name].append(split_data.drop_columns([dv for dv in all_data_variables if dv not in self.hparams.data_variables[series_name]]))
    ...
    def get_data(self, data_kwargs: List[str], main_split: str = "train") -> Dict[str, darts.timeseries.TimeSeries]:
        """
        Get dictionary of training and validation data (if defined) formatted with the names expected by the darts model
        methods such as fit, predict, historical_forecast, backtest, etc. I.e. intended use is for instance:
        model.fit(**datamodule.get_data(), other=value1, kwargs=value2).

        :param data_kwargs: List of strings specifying which data to return. The strings should be the same as the
            arguments of the model.fit method. The strings can be prefixed with "val_" to specify that the data should
            be returned for the validation split. If no prefix is given, the data is returned for the main split.
        :param main_split: String specifying which split should be used as the main split. The main split is used for
            all data that is not prefixed with "val_".
        ...
        for kwarg in data_kwargs:
            ...
            if kwarg in data_translator:
                if data_translator[kwarg] in getattr(self, f"data_{kwarg_split}"):
                    kwarg_value = darts.utils.utils.seq2series(getattr(self, f"data_{kwarg_split}")[data_translator[kwarg]])
                else:
                    kwarg_value = None
            ...
            elif hasattr(self.hparams, kwarg):  # TODO: is this safe? Meant for stuff like num_loader_workers, max_samples_per_ts
    ...
    def save_state(self, save_dir: str, backend: str = "pickle") -> None:
        """
        Save the state of the datamodule to a directory. This includes the state of the fitted processing pipeline such
        that data will be transformed in the same way as during training, even if a different data split is used.
        ...
        :return: None
        """
        assert backend in _SUPPORTED_SAVE_BACKENDS, f"The supported saving/loading backends are {_SUPPORTED_SAVE_BACKENDS}, you have {backend}"
    ...
    def load_state(self, load_dir: str, backend: str = "pickle") -> None:
        """
        Load the state of the datamodule from a directory. This includes the state of the fitted processing pipeline
        such that data will be transformed in the same way as during training, even if a different data split is used.
    ...
    def plot_data(self,
                  ...
                  slice: Optional[Tuple[Union[float, int, pd.Timestamp], Union[float, int, pd.Timestamp]]] = None,  # TODO: convert for user?
                  predictions: Optional[darts.timeseries.TimeSeries] = None,
                  separate_components: Optional[bool] = None,
                  presenter: Any = None,
                  **presenter_kwargs) -> Union[List[plt.Figure], None]:  # TODO: consider rewriting to using TSD to save space
        """
        ...
        :param separate_components: Whether to plot the components of the split separately. If None, components will be
        plotted separately if there are _PLOT_SEPARATE_MAX or fewer components, or together otherwise.
        ...
        if split is not None:
            return self._plot_data_split(split=split, slice=slice, predictions=predictions, separate_components=separate_components, presenter=presenter, **presenter_kwargs)
        else:
            figs = []
            for split in ["train", "val", "test"]:
                if self.has_split_data(split):
                    figs.extend(self._plot_data_split(split=split, slice=slice, predictions=predictions, separate_components=separate_components, presenter=presenter, **presenter_kwargs))  # TODO: what about predictions now? Dictionary?

            return figs

    def _plot_data_split(self,
                         split: str,
                         slice: Optional[Tuple[Union[float, int, pd.Timestamp], Union[float, int, pd.Timestamp]]] = None,
                         predictions: Optional[darts.timeseries.TimeSeries] = None,
                         separate_components: Optional[bool] = None,
                         presenter: Any = "savefig",
                         **presenter_kwargs) -> Union[List[plt.Figure], None]:
        """
                ...
                for series_i, series in enumerate(serieses):
                    ...
                    if separate_series_components:
                        for component_name in series.components.values:
                            figs.append(src.utils.plotting.plot_darts_timeseries(series[component_name], title=title_prefix + f" {component_name}", presenter=presenter, **presenter_kwargs))
                    else:
                        figs.append(src.utils.plotting.plot_darts_timeseries(series, title=title_prefix, presenter=presenter, **presenter_kwargs))

        return figs

    def call_function_on_univariate_series(self, function: Callable, series_to_call: Dict[str, Any], presenter: Any = False, **presenter_kwargs) -> Dict[str, Dict[str, Dict[str, Any]]]:
        """
        This function provides access to univariate components of the dataset splits. It can be used to call a function on the
        univariate series. If the function returns a figure, it can be presented using the presenter.
        ...
        res = {}
        for split_name in ["train", "val", "test"]:
            if (series_to_call == "all" or split_name in series_to_call) and self.has_split_data(split_name):
                split_series_to_call = "all" if series_to_call == "all" else series_to_call[split_name]
                res[split_name] = {}
                for series_type, serieses in getattr(self, f"data_{split_name}").items():
                    if serieses is not None and (split_series_to_call == "all" or series_type in split_series_to_call):
                        for series_i, series in enumerate(serieses):
                            component_series_to_call = "all" if split_series_to_call == "all" else split_series_to_call[series_type]
                            res[split_name][series_type] = {}
                            for component_name in series.components.values:
                                if component_series_to_call == "all" or component_name in component_series_to_call:
                                    res[split_name][series_type][component_name] = function(series.univariate_component(component_name))
                                    if presenter is not False:
                                        ...
                                        plt.title(title)
                                        res[split_name][series_type][component_name] = src.utils.plotting.present_figure(fig, presenter, **presenter_kwargs)  # TODO: should we overwrite or return a separate argument? i.e. return res, figs

        return res

    def get_split_range(self, split: str) -> Union[Tuple[Union[int, pd.Timestamp], Union[int, pd.Timestamp]], None]:  # TODO: fix seq
        """
        Returns the range of a data split, i.e. a tuple with (start_index, stop_index). If data has DatetimeIndex this
        function returns pd.Timestamp, if data has RangeIndex this function returns int-indexes, if the split has no
        data, this function returns None.
        :param split: which data split to get range for. One of ["train", "val", "test", "predict"]
        :return: range of data split.
        """
        assert split in _VALID_SPLIT_NAMES, f"split must be one of {_VALID_SPLIT_NAMES}, you have split."
        if split == "predict":
    ...
    def transform_data(self,
                       ...
                       fit_pipeline: bool = False) -> Union[darts.TimeSeries, Sequence[darts.TimeSeries]]:
        """
        A function to transform a dataset using a darts Pipeline object containing a set of transformers. If
        fit_pipeline is True, the pipeline is fitted to the dataset before transforming it. If fit_pipeline is False,
        the pipeline is only transformed if it has been fitted before. If the pipeline has not been fitted, the dataset
        is returned without transformation.

        :param dataset: The dataset to transform.
        :param pipeline: The pipeline to use for transformation. If None, the pipeline specified in the __init__
            method is used.
        ...
        if pipeline is None:
            if self.hparams.processing_pipeline is None:
                log.warning("Tried transforming dataset but no pipeline has been configured. Returning dataset.")
                return dataset
        ...
        if fit_pipeline:
            ...
            if not getattr(pipeline, "_fit_called", False):
                log.warning("Tried transforming dataset but pipeline has not been fitted. Returning dataset.")
                return dataset

        dataset = darts.utils.utils.series2seq(dataset)

        transformed = []
        for ds in dataset:
            # TODO: might be wrong if someone passed a pipeline?
            if not ds.width == self.data.width or any(ds.components != self.data.components):
                assert set(ds.components).issubset(
                    self.data.components), "transform_data was called with a ds that includes components that the processing pipeline was not fitted for."
                dummy_ds = self.data.slice_intersect(ds)
                component_indexes = [self.data.components.get_loc(comp) for comp in ds.components]
                dummy_ds_values = dummy_ds.all_values()
                dummy_ds_values[:, component_indexes, :] = ds.all_values()

                dummy_transformed = pipeline.transform(dummy_ds.with_values(dummy_ds_values))

                transformed.append(ds.with_values(dummy_transformed.all_values()[:, component_indexes, :]))
            else:
                transformed.append(pipeline.transform(ds))
    ...
    def inverse_transform_data(self,
                               dataset: Union[darts.TimeSeries, Sequence[darts.TimeSeries]],
                               pipeline: Optional[darts.dataprocessing.Pipeline] = None,
                               partial: bool = False) -> Union[darts.TimeSeries, Sequence[darts.TimeSeries]]:
        """
        A function to inverse transform a dataset using a darts Pipeline object containing a set of transformers. If
        all transformers in the pipeline are not invertible, the partial argument can be set to True to return the
        inverse transformation of the invertible transformers. If the pipeline has not been fitted, the dataset is
        returned without inverse transformation. If the pipeline is not invertible and partial is False, an error is
        raised.

        :param dataset: The dataset to inverse transform.
        :param pipeline: The pipeline to use for inverse transformation. If None, the pipeline specified in the __init__
            method is used.
        :param partial: Whether to return the inverse transformation of the invertible transformers if the pipeline is
            not invertible.
        ...
        if pipeline is None:
            if self.hparams.processing_pipeline is None:
                log.warning("Tried inverse transforming dataset but no pipeline has been configured. Returning dataset.")
                return dataset
            elif not getattr(self.hparams.processing_pipeline, "_fit_called", False):
                log.warning("Tried inverse transforming dataset but pipeline has not been fitted. Returning dataset.")
                return dataset

            pipeline = self.hparams.processing_pipeline

        if not partial and not pipeline.invertible():
            raise ValueError("Pipeline is not invertible, and the partial argument was not set True.")

        diff_transformer_indexes = [t_i for t_i, transformer in enumerate(pipeline._transformers) if isinstance(transformer, darts.dataprocessing.transformers.Diff)]
        if len(diff_transformer_indexes) > 0:
            ...
            if len(diff_transformer_indexes) > 1 or diff_transformer_indexes[0] != 0:
                assert pipeline._transformers[diff_transformer_indexes[0]]._fitted_params[0][2] + sum_lags * pipeline._transformers[diff_transformer_indexes[0]]._fitted_params[0][3] == dataset.start_time(), \
                ...
                original_dataset = self.data.slice_n_points_before(dataset.start_time(), sum_lags + 1)
        ...
        for ds in dataset:
            # TODO: might be wrong if someone passed a pipeline?
            if not ds.width == self.data.width or \
                (list(ds.components.values) != [str(i) for i in range(ds.width)] and any(ds.components != self.data.components)):
                assert set(ds.components).issubset(
                    self.data.components), "inverse_transform_data was called with a ds that includes components that the processing pipeline was not fitted for."
                # TODO: will this work if the data is outside the time index of the original self.data?
                dummy_ds = self.data.slice_intersect(ds)
                component_indexes = [self.data.components.get_loc(comp) for comp in ds.components]
                dummy_ds_values = dummy_ds.all_values()
                dummy_ds_values[:, component_indexes, :] = ds.all_values()

                dummy_transformed = pipeline.inverse_transform(dummy_ds.with_values(dummy_ds_values), partial=partial)

                transformed.append(ds.with_values(dummy_transformed.all_values()[:, component_indexes, :]))
            else:
                transformed.append(pipeline.inverse_transform(ds, partial=partial))
    ...
    def crop_dataset_range(
        ...
        :param crop_data_range: A list of two elements, the first being the start of the range, the second being the end.
            The elements can be strings, pd.Timestamps or ints. If strings, they are parsed as pd.Timestamps. If ints,
            they are interpreted as the number of points to crop from the start and end of the dataset.
        ...
        if isinstance(dataset, pd.DataFrame):
            dataset = TimeSeriesDataModule.subset_dataframe_on_index(dataset, crop_start, crop_stop)
    ...
    def resample_dataset(
        ...
        :param method: The method to use for resampling. Can be "interpolate" for which the dataset is interpolated in
            time to the new frequency, or any of the methods available in pd.DataFrame.resample.
        ...
        if isinstance(dataset, pd.DataFrame):
            assert isinstance(dataset.index, pd.DatetimeIndex), f"Can only resample data with pd.DatetimeIndex, you have {type(dataset.index)}"
            
            if method == "interpolate":
                resample_index = dataset.resample(freq).binner
                old_index = dataset.index
                dataset = dataset.reindex(old_index.union(resample_index)).interpolate(method="time").reindex(resample_index)
                ...
                if agg_func is None:
                    raise ValueError(f"The provided aggregation function is not a valid pandas dispatching function: {method}")
                dataset = agg_func()
        elif isinstance(dataset, darts.timeseries.TimeSeries):
    ...
    def process_train_val_test_split(
        ...
        """
        Takes in the train_val_test_split dictionary, asserts it has a valid structure, and processes it to a dictionary
        with the correct types. If applicable converts single value-style splits into list-style splits, i.e. if the
        train_val_test_split is {"train": 0.8, "val": 0.2} it will be converted to
        ...
        assert all([so in ["train", "test", "val"] for so in split_order]), "Only the entries [train, val, test] are allowed in train_val_test_split"
        split_values = list(train_val_test_split.values())

        # check that all split values are the same type
        assert type(split_values[0]) in _ALLOWED_SPLIT_TYPES, f"Split values must have one of the following types: {_ALLOWED_SPLIT_TYPES}"
        # TODO: works if multiple list splits are mixed with single list-splits?
        assert all([isinstance(sv, type(split_values[0])) for sv in split_values]), f"All split values must be same type, you have {[type(sv) for sv in split_values]}"
        ...
        if not isinstance(split_values[0], (list, tuple)):
            _assert_compatible_with_index(split_values[0], dataset.time_index)  # TODO: convert for user?
            if isinstance(split_values[0], float):
                assert sum(split_values) <= 1.0, f"You have provided split_values as floats, but they add up to more than 1 ({sum(split_values)})"
                prev_split = 0.0
            elif isinstance(split_values[0], int):
                assert sum(split_values) <= len(dataset), f"You have provided split_values as ints, but they add up to more than the number of elements in the dataset ({len(dataset)}"
                prev_split = 0
                ...
                if split_name in train_val_test_split:
                    if isinstance(split_values[0], (float, int)):
                        ...
                        if isinstance(this_split, str):
                            assert this_split not in ["start", "end"], "Can not use the special strings [start, end] when not supplying explicit [start_point, end_point] values for each split."
    ...
    def split_dataset(
        ...
        """
        Takes a dataset and a processed train_val_test_split dictionary and splits the dataset, return a dictionary with
        the split as keys and the split data as values. Ensure that the train_val_test_split dictionary has been
        processed by calling process_train_val_test_split before calling this function.

        :param dataset: darts.Timeseries object to be split
        :param train_val_test_split: Dictionary with keys split_name and values list of [start_index, stop_index] for
            that split.
        ...
        def process_split_values(_split_values):
            ...
            if split_i < len(split_order) - 1 and train_val_test_split[split_name][-1] == \
        ...
        split_order = list(train_val_test_split)
        for split_i, split_name in enumerate(train_val_test_split):  # TODO: use ordered_dict to ensure order?
            if split_name in train_val_test_split:
                split_values = train_val_test_split[split_name]
                if isinstance(split_values[0], list):
                    splits[split_name] = [dataset.slice(*process_split_values(split_v)) for split_v in split_values]
                else:
    ...
    def has_split_covariate_type(self, split: str, covariate_type: str) -> bool:
        """
        Returns True if the datamodule has the given covariate_type (past/future/static) for the specified split,
        False otherwise.

        :param split: The split to check for data.
        :param covariate_type: Type of covariate (past/future/static)
        :return: True if the datamodule has the covariate type for the specified split, False otherwise.
        """
        split_data = getattr(self, f"data_{split}")
