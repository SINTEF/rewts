from typing import List, Tuple, Optional, Dict, Any, Callable, Union, Sequence
...
import datetime
import platform
from src.models.ensemble_model import TSEnsembleModel
import pandas as pd
...
def has_valid_extension(fig: plt.Figure, fname: str):
    """
    Helper function to check if the given filename has a valid extension/file format supported by the current active
    matplotlib backend.
    :param fig: Figure from which the supported filetypes are extracted.
    :param fname: Filename to check

    :return: True if fname has a valid extension supported by the active matplotlib backend, False otherwise
    """
...
def plot_darts_timeseries(
    ...
    **presenter_kwargs,
    ...
    """
    Plots a darts TimeSeries object. If the series is a sequence of TimeSeries objects, plots each series in a separate
    figure.

    :param series_to_be_plotted: TimeSeries object to be plotted.
    :param title: Title of the plot.
    :param presenter: Presenter to use for plotting. Can be a pytorch_lightning logger, a string ["savefig", "show"] or
        None.
    ...
    for series_i, series in enumerate(series_list):
        ...
        if isinstance(
            presenter, pytorch_lightning.loggers.tensorboard.TensorBoardLogger
        ):
            stage_plot_kwargs.update(dict(tag=title))
        elif "fname" in stage_plot_kwargs:
            stage_plot_kwargs["fname"] = os.path.join(
                stage_plot_kwargs["fname"], f"{title}"
            )
...
def plot_prediction(
    ...
    """
    Plots prediction(s) of a model versus the actual target series as well as any covariates consumed by the model.

    :param prediction: Prediction to plot.
    :param prediction_data: Dictionary containing the actual target series and any covariates consumed by the model.
    :param model: Model that produced the prediction.
    :param presenters: Presenter(s) to use for plotting. Can be a single presenter or a list of presenters where
        each presenter is a pytorch_lightning logger, a string ["savefig", "show"] or None.
    :param predict_n: The forecast_horizon used to produce the prediction.
    :param plot_covariates: Whether to plot the covariates of the model.
    :param plot_encodings: Whether to plot the encodings of the model. Note that plot_covariates must also be True.
    ...
    if presenter_kwargs is not None:
        assert len(presenters) == len(presenter_kwargs) and type(presenters) == type(presenter_kwargs)
    ...
    for data_type in ["series", "past_covariates", "future_covariates"]:
        if prediction_data.get(data_type) is None or ("covariates" in data_type and not plot_covariates):
            plot_data[data_type] = None
        else:
            if prediction[-1].start_time() > prediction_data[data_type].end_time():
                assert prediction[-1].start_time() == prediction_data[data_type].end_time() + prediction_data[data_type].freq
            ...
            if model.extreme_lags[lag_data_type_translator[data_type]] is not None:
                start_idx -= abs(
                    model.extreme_lags[lag_data_type_translator[data_type]]
                )
            ...
            if prediction[-1].end_time() > prediction_data[data_type].end_time():
                ...
                end_idx = prediction_data[data_type].get_index_at_point(
                    prediction[-1].end_time()
                )
                if (
                    data_type == "future_covariates"
                    and model.extreme_lags[lag_data_type_translator[data_type] + 1]
                    is not None
                ):
                    end_idx += abs(
                        model.extreme_lags[lag_data_type_translator[data_type] + 1] + 1
                    )
            ...
            if isinstance(prediction_data[data_type].time_index, pd.DatetimeIndex):
                ...
                raise ValueError(f"Not supported time_index type {type(prediction_data[data_type].time_index)}")

    if inverse_transform_data_func is not None:
        assert callable(inverse_transform_data_func)
        try:
            prediction = [inverse_transform_data_func(pred) for pred in prediction]
            for data_type in plot_data:
                if plot_data[data_type] is not None:
                    plot_data[data_type] = inverse_transform_data_func(
                        plot_data[data_type]
                    )
        except Exception as e:
            log.exception(
                "Inverse transform of datamodule pipeline failed with the following error"
            )
    ...
    if plot_weights:
        if not isinstance(model, TSEnsembleModel):
            log.warning("plot_weights can only be used with the TSEnsembleModel. Setting argument to false")
            plot_weights = False
        elif len(model._weights_history) == 0:
            log.warning("plot_weights was True but TSEnsembleModel weight history is empty. Ensure weights have been fitted and model has not been reset.")
            plot_weights = False
        else:
            nplots += 1
    fig, axs = create_figure(nplots, 1, sharex=not plot_weights, figsize=(6.4, 4.8 * nplots))
    plot_i = 0
    for data_type in plot_data:
        if plot_data[data_type] is not None:
            plt.sca(axs[plot_i])
            if data_type == "series":
                plot_data["series"].plot(label="actual")
                axs[plot_i].set_title("target")
                if plot_prediction_point:
                    if prediction[0].start_time() > plot_data["series"].end_time():
                        assert prediction[0].start_time() == plot_data["series"].end_time() + plot_data["series"].freq
                        # TODO: is this assert necessary? We require that prediction starts maximum one step after the end of the data
                        prediction_point_index = len(plot_data["series"]) - 2
                    else:
                        prediction_point_index = plot_data["series"].get_index_at_point(
                            prediction[0].start_time()
                        ) - 1
                    axs[plot_i].axvline(
                        plot_data["series"].time_index[prediction_point_index],
                    )
                # Do not reuse the first color in the color cycle, as it is already used by the actual line (only true for univariate...)
                color_cycle = plt.rcParams['axes.prop_cycle'].by_key()['color']
                for pred_i, pred in enumerate(prediction):
                    if len(prediction) == 1:
                        pred.plot(label="prediction")
                    else:
                        pred.plot(label="_nolegend_", color=color_cycle[1 + pred_i % (len(color_cycle) - 1)])
            else:
                plot_data[data_type].plot()
                axs[plot_i].set_title(data_type)
            plot_i += 1
    if plot_weights:
        plot_ensemble_weights(model._weights_history, axs[-1])
...
def present_figure(
    ...
    """
    Utility function to present a figure using a given presenter. If presenter is None, the figure is returned. If the
    presenter is a TensorBoardLogger, the figure is added to the TensorBoardLogger. If the presenter is a MLFlowLogger,
    the figure is saved to the MLFlowLogger. If the presenter is show, the figure is shown. If the presenter is savefig,
    the figure is saved to the path given in the fname keyword argument. If presenter is not None, the figure is closed.
    ...
    if presenter is None:
        return fig
    elif isinstance(presenter, pytorch_lightning.loggers.tensorboard.TensorBoardLogger):
        ...
        raise ValueError(
            f"Unrecognized presenter {presenter}, must be one of {PRESENTERS}"
        )
    plt.close(fig)
    
    
def plot_ensemble_weights(weights, ax=None, time_indices=None, presenter: Any = None, **presenter_kwargs):
    ...
    if len(weights.shape) == 1:  # bar plot
        ...
        if len(model_indices) > 20:
            ...
            key_labels = ['M{}'.format(i) for i in key_indices]
            cbar.set_ticklabels(key_labels)
        else:
            # Set the range of the color bar
            cbar.set_ticks(np.linspace(min(model_indices), max(model_indices), num=len(model_indices)))
